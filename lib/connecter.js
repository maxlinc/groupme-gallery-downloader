...
import find_groups from './find_groups';
...
function parsePhotos(data) {
  return data.map(photo => {
    for (let i = 0; i < photo.attachments.length; i++) {
      let theUrl = photo.attachments[i].url;
      if (typeof theUrl !== "string") {
        console.log("Skipping attachment without url:");
        console.log(photo.attachments[i]);
        continue;
      }
      console.log("Found " + theUrl)
      return photo.attachments[i].url;
    }
    return null;
  });
}
...
export default async function (token, id) {
  galleryConnect(token, id, data => {
    downloader(parsePhotos(filterPhotos(data)), id);
  });
}
